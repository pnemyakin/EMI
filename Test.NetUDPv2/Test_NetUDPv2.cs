...
{
    [TestClass]
    public class Test_NetUDPv2
    {
        /// <summary>
        /// Порт увеличивается с каждым тестом чтобы избежать конфликтов
        /// </summary>
        private static int PortCounter = 41100;
        ...
        private class TestEnv : IDisposable
        {
            ...
            public static async Task<TestEnv> CreateAsync(int timeoutMs = 5000)
            {
                ...
                env.Server = NetUDPv2Service.Service.GetNewServer();
                env.Server.StartServer(env.Address);

                env.Client = NetUDPv2Service.Service.GetNewClient();
        ...
        {
            var client = NetUDPv2Service.Service.GetNewClient();
            var cts = new CancellationTokenSource(3000);

            bool connected = await client.Connect("localhost#39989", cts.Token);
        ...
        public async Task MultipleClients_Connect()
        {
            ...
            var server = NetUDPv2Service.Service.GetNewServer();
            ...
            try
            {
                ...
                {
                    clients[i] = NetUDPv2Service.Service.GetNewClient();
        ...
        public async Task SendReceive_Unreliable_MaxSinglePacket()
        {
            ...
            int size = V2Constants.MaxUnreliablePayload;
        ...
        public async Task SendReceive_Unreliable_Fragmented()
        {
            ...
            int size = V2Constants.DefaultMTU * 3;
            byte[] data = new byte[size];
            for (int i = 0; i < size; i++)
                data[i] = (byte)(i % 256);
        ...
        public async Task SendReceive_Reliable_Fragmented()
        {
            ...
            int size = V2Constants.DefaultMTU * 5;
            byte[] data = new byte[size];
            for (int i = 0; i < size; i++)
        ...
        public async Task Throughput_ManySmallUnreliable()
        {
            ...
            // Получаем (на localhost все должны дойти)
            int receivedCount = 0;
            for (int i = 0; i < packetCount; i++)
            {
        ...
        [TestMethod]
        public async Task Throughput_BurstReliable()
        {
            using var env = await TestEnv.CreateAsync();
            var cts = new CancellationTokenSource(30000);
            ...
            // Отправляем все reliable пакеты burst-ом
            for (int i = 0; i < packetCount; i++)
            {
                byte[] data = new byte[] { (byte)(i + 1), 0xDE, 0xAD };
                await env.Client.Send(new EasyArray(data), true, cts.Token);
            }
            ...
            Assert.AreEqual(packetCount, receivedCount,
                $"All reliable packets must be delivered: {receivedCount}/{packetCount}");
        }
        ...
        [TestMethod]
        public async Task DeliveredRate_AfterTraffic()
        {
            using var env = await TestEnv.CreateAsync();
            var cts = new CancellationTokenSource(10000);

            // Шлём пачку reliable → rate должен быть высоким
            for (int i = 0; i < 20; i++)
            {
                byte[] d = { (byte)i };
                await env.Client.Send(new EasyArray(d), true, cts.Token);
                var r = await env.ServerClient.AcceptPacket(1024, cts.Token);
                r.Dispose();
            }

            // На localhost без потерь rate должен быть хорошим
            Assert.IsTrue(env.Client.DeliveredRate >= 0.5f,
                $"DeliveredRate {env.Client.DeliveredRate} too low for localhost");
        }
        ...
        [TestMethod]
        public void SendByteSpeed_DefaultValue()
        {
            var client = NetUDPv2Service.Service.GetNewClient();
            Assert.AreEqual(0L, client.SendByteSpeed);
        }
        ...
        [TestMethod]
        public async Task SendReceive_SingleByte()
        {
            using var env = await TestEnv.CreateAsync();
            var cts = new CancellationTokenSource(5000);

            await env.Client.Send(new EasyArray(new byte[] { 0xFF }), true, cts.Token);

            var received = await env.ServerClient.AcceptPacket(1024, cts.Token);
            Assert.AreEqual(1, received.Length);
            Assert.AreEqual(0xFF, received.Bytes[0]);
            received.Dispose();
        }

        [TestMethod]
        public async Task MultipleFragmentedMessages_Sequential()
        {
            using var env = await TestEnv.CreateAsync();
            var cts = new CancellationTokenSource(20000);

            // Отправляем несколько больших сообщений подряд
            for (int msg = 0; msg < 3; msg++)
            {
                int size = V2Constants.DefaultMTU * 3;
                byte[] data = new byte[size];
                for (int i = 0; i < size; i++)
                    data[i] = (byte)((i + msg * 17) % 256);

                await env.Client.Send(new EasyArray(data), true, cts.Token);

                var received = await env.ServerClient.AcceptPacket(size + 100, cts.Token);
                Assert.AreEqual(size, received.Length, $"Message {msg} size mismatch");

                for (int i = 0; i < size; i++)
                    Assert.AreEqual(data[i], received.Bytes[i], $"Msg {msg} byte {i} mismatch");
        ...
        public async Task Stress_ConcurrentSendReceive()
        {
            ...
            await Task.WhenAny(receiveTask, Task.Delay(15000));

            Assert.IsTrue(receivedCount > 0, "Should receive at least some messages");
        }

        [TestMethod]
        public async Task Stress_RapidConnectDisconnect()
        {
            var address = GetAddress();
            var server = NetUDPv2Service.Service.GetNewServer();
            server.StartServer(address);
            ...
            try
            {
                for (int cycle = 0; cycle < 3; cycle++)
                {
                    ...
                    bool connected = await client.Connect(address, cts.Token);
                    if (!connected)
                    {
                        // Иногда порт может не освободиться мгновенно — не страшно
                        continue;
                    }

                    var sc = await acceptTask;
                    Assert.IsTrue(client.IsConnect);

                    // Немного трафика
                    await client.Send(new EasyArray(new byte[] { (byte)cycle }), true, cts.Token);
                    var r = await sc.AcceptPacket(1024, cts.Token);
                    r.Dispose();
        ...
        #region Congestion Integration Tests

        [TestMethod]
        public async Task CongestionWindow_GrowsUnderLoad()
        {
            using var env = await TestEnv.CreateAsync();
            var cts = new CancellationTokenSource(20000);

            // Шлём много reliable данных — CWND должен вырасти
            for (int i = 0; i < 40; i++)
            {
            ...
            // Клиент: DeliveredRate должен быть высоким после успешной работы
            Assert.IsTrue(env.Client.DeliveredRate >= 0.5f,
                $"DeliveredRate {env.Client.DeliveredRate} should be healthy after clean traffic");
        }

        [TestMethod]
        public async Task DeliveredRate_StaysHealthyOnLocalhost()
        {
            using var env = await TestEnv.CreateAsync();
            var cts = new CancellationTokenSource(15000);
            ...
            for (int i = 0; i < 30; i++)
            {
                byte[] data = new byte[] { (byte)(i + 1) };
                await env.Client.Send(new EasyArray(data), true, cts.Token);
                var r = await env.ServerClient.AcceptPacket(1024, cts.Token);
                r.Dispose();

                float rate = env.Client.DeliveredRate;
                if (rate < minRate) minRate = rate;
                if (rate > maxRate) maxRate = rate;
            }

            Assert.IsTrue(minRate >= 0f && maxRate <= 1f,
                $"DeliveredRate range [{minRate}, {maxRate}] should be in [0, 1]");
        }
