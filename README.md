...
**RPC** : регистрация и вызов удалённых методов с поддержкой до 100 параметров и возвращаемых значений. Гарантированная и негарантированная доставка, серверное перенаправление вызовов между клиентами.
...
**Middleware** : конвейер обработки пакетов. Встроены LZ4-сжатие и AES-256-GCM шифрование. Ключ согласуется через сменяемую стратегию обмена (лестница уровней: off / PSK / RSA / RSA+пиннинг) и **не передаётся открытым текстом**. См. [docs/security.md](docs/security.md).
...
**Headless** : RPC-движок без встроенного транспорта — bring-your-own-transport. Для любого внешнего канала (Steam P2P, релей, WebSocket, движковый netcode, in-memory для тестов), где доставку байтов обеспечивает пользователь. Шифрование — PSK или полагаемся на шифрование внешнего транспорта.
